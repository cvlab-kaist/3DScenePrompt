...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--outdir", type=str, default="outputs_cvd", help="outputs direcotry"
  )
  parser.add_argument("--base_path", type=str, help="scene name")
  ...
  for _, folder_name in enumerate(folder_names):
        try:
            ...
            input_path = os.path.join(input_base_path, folder_name, 'megasam')
            output_dir = os.path.join(output_base_path, folder_name, 'megasam_refine')
            cache_dir = os.path.join(input_base_path, folder_name, 'cache_flow')
            scene_name = folder_name
            
        
            print(f'gpu : {args.batch} percentage : {_}/{len(folder_names)}')
            print(f"gpu : {args.batch} output_dir : {output_dir}")
            if os.path.exists(f"{output_dir}/{scene_name}_sgd_cvd_hr.npz"):
                print(f"already processed {scene_name}")
                continue
            
            print("***************************** ", scene_name)
            img_data = np.load(os.path.join(input_path, "images.npy"))[
                :, ::-1, ...
            ]

            disp_data = (
                np.load(
                    os.path.join(input_path, "disps.npy")
                )
                + 1e-6
            )
            intrinsics = np.load(os.path.join(input_path, "intrinsics.npy"))
            poses = np.load(os.path.join(input_path, "poses.npy"))
            mot_prob = np.load(os.path.join(input_path, "motion_prob.npy"))

            flows = np.load(
                "%s/flows.npy" % cache_dir, allow_pickle=True
            )
            flow_masks = np.load(
                "%s/flows_masks.npy" % cache_dir, allow_pickle=True
            )
            flow_masks = np.float32(flow_masks)
            iijj = np.load("%s/ii-jj.npy" % cache_dir, allow_pickle=True)
