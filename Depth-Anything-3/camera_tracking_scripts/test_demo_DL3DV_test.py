...
def save_full_reconstruction(
    droid, full_traj, rgb_list, senor_depth_list, motion_prob, scene_name, save_base_dir
):
  ...
  Path(f"{save_base_dir}/{scene_name}/megasam/").mkdir(
      parents=True, exist_ok=True
  )
  np.save(f"{save_base_dir}/{scene_name}/megasam/images.npy", images)
  np.save(f"{save_base_dir}/{scene_name}/megasam/disps.npy", disps)
  np.save(f"{save_base_dir}/{scene_name}/megasam/poses.npy", poses)
  np.save(
      f"{save_base_dir}/{scene_name}/megasam/intrinsics.npy", intrinsics * 8.0
  )
  np.save(f"{save_base_dir}/{scene_name}/megasam/motion_prob.npy", motion_prob)
...
#   max_frames = min(1000, images.shape[0])
  max_frames = images.shape[0]
#   print("outputs/%s_droid.npz" % scene_name)
  print(f"{save_base_dir}/{scene_name}/megasam/{scene_name}.npz")
#   Path("outputs").mkdir(parents=True, exist_ok=True)

  np.savez(
      f"{save_base_dir}/{scene_name}/megasam/{scene_name}.npz",
...
if __name__ == "__main__":
  ...
  if args.batch!=  -1:
      ...
      folder_names = folder_names[int(args.batch/5* data_len):int((args.batch+1)/5* data_len)]
      
  for _, scene_name in enumerate(folder_names):   
    time1 = time.time()
    print(f'gpu : {args.batch} percentage : {_}/{len(folder_names)}')
    print(f"gpu : {args.batch} output_dir : {out_base_path}/{scene_name}/megasam")
    if os.path.exists(f"{out_base_path}/{scene_name}/megasam/{scene_name}.npz"):
        print(f"already processed {scene_name}")
        continue
    # try:
    input_dir = os.path.join(input_base_path, scene_name, 'color')
    mono_depth_path = os.path.join(input_base_path, scene_name, 'Depth-Anything')
    metric_depth_path = os.path.join(input_base_path, scene_name, 'UniDepth')

    tstamps = []
    rgb_list = []
    senor_depth_list = []

    image_list = sorted(glob.glob(os.path.join("%s" % (input_dir), "*.jpg")))
    image_list += sorted(glob.glob(os.path.join("%s" % (input_dir), "*.png")))

    # NOTE Mono is inverse depth, but metric-depth is depth!
    mono_disp_paths = sorted(
        glob.glob(
            os.path.join(mono_depth_path, "*.npy")
        )
    )
    metric_depth_paths = sorted(
        glob.glob(
            os.path.join(metric_depth_path, "*.npz")
        )
    )
    ...
    time2 = time.time()
    print(f"gpu : {args.batch} total time ", time2 - time1)
    
    del droid
    del rgb_list
    del senor_depth_list
    del mono_disp_list
    del image_list
    del metric_depth_paths
    del mono_disp_paths
    del image
    del depth
    del intrinsics
