...
def init_attention(attn: "Attention"):
    """
    Initialize weights for an Attention module.
    - q, k, out : Xavier init
    - v         : zero init
    ...
    for name, module in attn.named_modules():
        if isinstance(module, nn.Linear):
            if "to_v" in name:  # value projection은 0으로 초기화
...
def init_layernorm_zero(norm: "CogVideoXLayerNormZero"):
    """
    Initialize CogVideoXLayerNormZero:
    - weight to zero
    - bias to zero (if exists)
    """
...
class PluckerEncoder(nn.Module):
    def __init__(self):
        ...
        self.proj = nn.Sequential(
            nn.Conv3d(in_channels=6, out_channels=32, kernel_size=(4, 8, 8), stride=(4, 8, 8), padding=0),  # [B, 32, 13, 60, 90]
            nn.BatchNorm3d(32),
    ...
    def forward(self, x):
        ...
        # 앞쪽 temporal padding 3프레임 추가 → [B, C, 52, H, W]
        x = F.pad(x, (0, 0, 0, 0, 3, 0))  # pad temporal dim (dim=2) with 3 at the front

        x = self.proj(x)  # → [B, 16, 13, 60, 90]
        x = x.permute(0, 2, 1, 3, 4)  # → [B, 13, 16, 60, 90]
        return x
...
class CogVideoXCrossAttnProcessor2_0:
    r"""
    ...
    def __call__(
        ...
        key = attn.to_k(cut3r_state)
        value = attn.to_v(cut3r_state)
...
class LayerNormZero(nn.Module):
    def __init__(
        ...
        # self.norm2 = nn.BatchNorm1d(embedding_dim2, eps=eps)
...
class CogVideoXDASBlock(CogVideoXBlock):
    ...
    def __init__(
        ...
        self.cross_attn = Attention(
            ...
            cross_attention_norm="layer_norm" if qk_norm else None,
        ...
        self.cross_attn.to_v.weight.data.zero_()
        self.cross_attn.to_v.bias.data.zero_()
        
        
        self.norm3 = LayerNormZero(time_embed_dim, dim, 768, norm_elementwise_affine, norm_eps, bias=True)
        self.norm3.linear.weight.data.zero_()
        self.norm3.linear.bias.data.zero_()
        # init_attention(self.cross_attn)
        # init_layernorm_zero(self.norm3)
    ...
    def forward(
        ...
        cut3r_state = cut3r_state.to(hidden_states.device, torch.bfloat16).squeeze(dim=1)
        # norm & modulate
        ...
        norm_hidden_states, gate_cross = self.norm3(
            hidden_states, temb
        )
        
        attn_hidden_states, attn_encoder_hidden_states = self.cross_attn(
            hidden_states=norm_hidden_states,
        ...
        hidden_states = hidden_states + gate_cross * attn_hidden_states
        # encoder_hidden_states = encoder_hidden_states + enc_gate_cross * attn_encoder_hidden_states
...
@maybe_allow_in_graph
class CogVideoXTransformer3DModelTracking_cut3r(CogVideoXTransformer3DModel, ModelMixin):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.camera_embedder = PluckerEncoder()
        ...
        # self.camera_embedder = nn.ModuleList(
        #     [
        #         nn.Linear(16,64, device="cpu")
        #         for _ in range(num_tracking_blocks)
        #     ]
        # )
        # self.camera_embedder[0].weight.data.zero_()
        # self.camera_embedder[0].bias.data.zero_()
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], load_from_cogvideo=False, **kwargs):
        if load_from_cogvideo:
            ...
            for i in range(model.num_tracking_blocks):
                ...
                model.transformer_blocks_copy[i].cross_attn.to_v.weight.data.zero_()
                model.transformer_blocks_copy[i].cross_attn.to_v.bias.data.zero_()
                model.transformer_blocks_copy[i].norm3.linear.weight.data.zero_()
                model.transformer_blocks_copy[i].norm3.linear.bias.data.zero_() 
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if is_main_process:
            ...
            config_dict["_class_name"] = "CogVideoXTransformer3DModelTracking_cut3r"
