...
class CogVideoXV2VDASPipeline(CogVideoXImageToVideoPipeline, DiffusionPipeline):
    ...
    def prepare_latents(
        ...
        video = video.unsqueeze(0)  # [B, F, C, H, W]
        ...
        if isinstance(generator, list):
            ...
            shrot_video_latents = [retrieve_latents(self.vae.encode(vid.unsqueeze(0)), generator) for vid in video]

        shrot_video_latents = torch.cat(shrot_video_latents, dim=0).to(dtype).permute(0, 2, 1, 3, 4)  # [B, F, C, H, W]
        shrot_video_latents = self.vae_scaling_factor_image * shrot_video_latents
        shrot_video_latents = shrot_video_latents[:, :3, :, :, :]
        ...
        shrot_video_latents = torch.cat([shrot_video_latents, latent_padding], dim=1)
        ...
        # shrot_video_latents = self.scheduler.add_noise(shrot_video_latents, noise, cond_timestep)

        # scale the initial noise by the standard deviation required by the scheduler
        latents = latents * self.scheduler.init_noise_sigma
        return latents, shrot_video_latents
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # tracking_maps_input = torch.cat([camera_embbeding_temp, latent_video_input], dim=2)
                del latent_video_input
