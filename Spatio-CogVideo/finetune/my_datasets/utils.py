...
def load_cameras(camera_path: Path) -> List[Path]:
    with open(camera_path, "r", encoding="utf-8") as file:
        return [camera_path.parent / line.strip() for line in file.readlines() if len(line.strip()) > 0]
...
def load_short_videos_from_videos(videos_path: List[Path], cond_video_length: int =5) -> List[Path]:
    first_videos_dir = videos_path[0].parent.parent / "short_video"
    first_videos_dir.mkdir(exist_ok=True)
    import os
    if os.path.exists(first_videos_dir) and len(os.listdir(first_videos_dir)) == len(videos_path):
        first_video_paths = [first_videos_dir / f"{video_path.stem}.mp4" for video_path in videos_path]
        return first_video_paths
    
    first_video_paths = []
    for video_path in tqdm(videos_path, desc="Processing videos"):
        output_video_path = first_videos_dir / f"{video_path.stem}.mp4"
        if output_video_path.exists():
            first_video_paths.append(output_video_path)
            continue
        ...
        for i, frame in enumerate(reader):
            if i >= cond_video_length:
                break
            frames.append(frame)

        if not frames:
            logging.warning(f"No frames read from {video_path}")
            continue

        # Save as video
        writer = imageio.get_writer(output_video_path, fps=fps, codec='libx264', macro_block_size=16)
        for frame in frames:
            writer.append_data(frame)
        writer.close()
        logging.info(f"Saved first {cond_video_length} frames as a video to {output_video_path}")
...
def preprecess_plucker(camera_path: Path | str, height: int, width: int) -> torch.Tensor:
    """
    Loads and resizes a single camera.
    ...
    Returns:
        torch.Tensor: Camera tensor with shape [C, H, W] where:
            C = number of channels (3 for RGB)
            H = height
            W = width
    """
