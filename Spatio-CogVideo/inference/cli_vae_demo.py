# python cli_vae_demo.py --model_path THUDM/CogVideoX-5b-I2V --encoded_path ../finetune/cond.pt --mode decode
...
def encode_video(model_path, video_path, dtype, device):
    ...
    model = AutoencoderKLCogVideoX.from_pretrained(model_path, subfolder="vae").to(device)
...
def decode_video(model_path, encoded_tensor_path, dtype, device):
    ...
    """
    # model = AutoencoderKLCogVideoX.from_pretrained(model_path, torch_dtype=dtype).to(device)
    model = AutoencoderKLCogVideoX.from_pretrained(model_path, subfolder="vae").to(device).to(dtype)
    encoded_frames = torch.load(encoded_tensor_path, weights_only=True).to(device).to(dtype)
    encoded_frames =  1 / model.config.scaling_factor * encoded_frames
    encoded_frames = encoded_frames.permute(0,2,1,3,4)
    with torch.no_grad():
        decoded_frames = model.decode(encoded_frames).sample * 0.5 + 0.5
